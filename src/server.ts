...
app.get('/dracos/', async (req, res) => {
  const dir = await promises.opendir(path.join(__dirname, 'asset/dracos/'));
  const files: string[] = [];
  // eslint-disable-next-line no-restricted-syntax
  for await (const dirent of dir) {
    files.push(dirent.name);
...
app.get('/dracos/:id', async (req, res) => {
  const filePath = path.join(__dirname, `asset/dracos/${req.params.id}`);
  const result = await promises.readFile(filePath);
  res.send(result);
});
